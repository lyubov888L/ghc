...
tcPatToExpr name args pat = go pat
  where
    ...
    mkRecordConExpr con fields
      = do { exprFields <- go' fields
           ; return (RecordCon noExtField con exprFields) }

    go' :: HsRecFields GhcRn (LPat GhcRn) -> Either SDoc (HsRecordBinds GhcRn)
    go' (HsRecFields fields mdd) = do
      fields' <- mapM go'' fields
      return (HsRecFields fields' mdd)

    go'' :: LHsRecField GhcRn (LPat GhcRn) -> Either SDoc (LHsRecField GhcRn (LHsExpr GhcRn))
    go'' (L loc p) = L loc <$> traverse go p
