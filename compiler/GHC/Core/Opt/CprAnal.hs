...
giveStrictFieldsCpr env dc bndrs = foldl' do_one_field env (fields_w_dmds dc)
  where
    -- 'extendSigEnvForDemand' is Test (C) of Note [Optimistic field binder CPR]
...
thus reboxing in the wrapper:
  ...
  (c) A strict field binder doesn't mean the function is strict in it.
...
But it's simple, and nothing terrible happens if we get it wrong. e.g. #10694.

We similarly used to give the case binder the CPR property unconditionally
